...
@section('content')
    ...
    <div>
        <a href="{{ route('stadia.create') }}"><button
                ...
                <table class="min-w-full leading-normal table-fixed">
                    <thead>
                        <tr>
                            ...
                            <th
                                class="px-5 py-3 border-b-2 border-gray-200 bg-lime-50 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                Capacitat
                            </th>
                            <th
                                class="px-5 py-3 border-b-2 border-gray-200 bg-lime-50 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                        ...
                        @foreach ($stadia as $stadium)
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        <a
                                            href="{{ route('stadia.show', $stadium->id) }}">{{ $stadium->team->name ?? '(sense equip asignat)' }}</a>
                                    </p>
                                </td>
                                ...
                                <td class="px-7 py-5 border-b border-gray-200 bg-white text-sm ">
                                    <form action="{{ route('stadia.destroy', $stadium) }}" method="POST">
                                        @csrf
                                        @method('delete')
                                        <button type="submit"
