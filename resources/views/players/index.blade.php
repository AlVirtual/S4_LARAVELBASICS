...
@section('content')
    ...
    <div>
        <a href="{{ route('players.create') }}"><button
                class="px-6 py-2 mx-auto block rounded-md text-lg font-semibold text-white bg-lime-700 hover:bg-lime-800">
                Crear Jugador
            </button></a>
    </div>
                ...
                <table class="min-w-full leading-normal table-fixed">
                    <thead>
                        <tr>
                            ...
                            <th
                                class="w-1/2 px-5 py-3 border-b-2 border-gray-200 bg-lime-50 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                Equip
                            </th>
                            <th
                    ...
                    <tbody>
                        @foreach ($players as $player)
                            <tr>
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        <a href="{{ route('players.show', $player->id) }}">{{ $player->surname }}</a>
                                    </p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        <a href={{ route('players.show', $player->id) }}>{{ $player->age }} </a>
                                    </p>
                                </td>
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    <p class="text-gray-900 whitespace-no-wrap">
                                        <a href="{{ route('players.show', $player->id) }}">{{ $player->team->name }}</a>
                                    </p>
                                </td>
                                ...
                                <td class="px-7 py-5 border-b border-gray-200 bg-white text-sm ">
                                    <form action="{{ route('players.destroy', $player) }}" method="POST">
                                        ...
                                        @method('delete')
                                        <button type="submit"
                                            class="px-6 py-2 mx-auto block rounded-md text-lg font-semibold">
